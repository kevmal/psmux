    ...
    match option {
        ...
        "warm" => {
            ...
            if !app.warm_enabled {
                ...
                if let Some(port) = crate::registry::read_port(&warm_base) {
                ...
                // The killed server removes its own registration
                // (compare-before-delete); only scrub here if it is
                // genuinely dead and left files behind.
                if crate::registry::registration_liveness(&warm_base) == crate::registry::Liveness::Dead {
